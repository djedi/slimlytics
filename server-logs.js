...
async function main() {
	console.log(
		`${colors.bright}${colors.cyan}Slimlytics Server Logs & Debugging${colors.reset}\n`,
	);
	...
	try {
		...
		await sshExecStream(
			conn,
			`cd ${server.path} && docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"`,
		);
		...
		await sshExecStream(
			conn,
			`cd ${server.path} && docker compose logs --tail=50 slimlytics`,
		);
		...
		await sshExecStream(
			conn,
			`cd ${server.path} && docker compose logs --tail=30 caddy`,
		);
		...
		await sshExecStream(
			conn,
			`cd ${server.path} && docker compose exec slimlytics curl -f http://localhost:3000/health || echo "Health check failed"`,
		);
		...
		await sshExecStream(
			conn,
			`ls -lah ${server.path}/data/analytics.db 2>/dev/null || echo "Database file not found"`,
		);
		...
		await sshExecStream(
			conn,
			`test -f ${server.path}/.env && echo ".env file exists" || echo ".env file missing!"`,
		);
		...
		switch (choice.trim()) {
			case "1":
				...
				await sshExecStream(
					conn,
					`cd ${server.path} && docker compose logs -f slimlytics`,
				);
				break;
			...
			case "2":
				...
				await sshExecStream(
					conn,
					`cd ${server.path} && docker compose logs -f caddy`,
				);
				break;
			...
			case "3":
				...
				await sshExecStream(
					conn,
					`cd ${server.path} && docker compose logs -f`,
				);
				break;
			...
			case "4":
				...
				await sshExecStream(
					conn,
					`cd ${server.path} && docker compose restart`,
				);
			...
			case "9":
				...
				await sshExecStream(
					conn,
					`cd ${server.path} && docker compose exec slimlytics ls -la /app/data/`,
				);
				await sshExecStream(
					conn,
					`cd ${server.path} && docker compose exec slimlytics sqlite3 /app/data/analytics.db "SELECT COUNT(*) as sites FROM sites;" 2>&1 || echo "Database query failed"`,
				);
				break;
			...
			case "10":
				...
				await sshExecStream(
					conn,
					`cd ${server.path} && cat .env | sed 's/=.*/=***MASKED***/'`,
				);
				break;
